...
router.get('/alltraders', (req, res) => {
    try {
        ...
        var query = `SELECT * FROM users where uid in (SELECT uid FROM users WHERE key = '${dbconstants.DB_USERTYPE}' AND value = '${text.TEXT_TRADER}' ORDER BY uid OFFSET ${index} limit 10)`;
